...
def layered_summon_task_lora_params(fsdp_module, adapter_name: str) -> OrderedDict:
    from peft.utils.save_and_load import get_peft_model_state_dict
...
def collect_task_lora_params(module: FSDP, layered_summon: bool, base_sync_done: bool, adapter_name: str) -> OrderedDict:
    """
    collect lora params or full params if base model is not ready in vllm
    work with if isinstance(self.module._fsdp_wrapped_module, PeftModel)
    """
    ...
    if fsdp_version(module) > 0:
        if layered_summon:
            ...
            lora_params = layered_summon_task_lora_params(module, adapter_name)
        else:
            with FSDP.summon_full_params(module, writeback=False):
                if base_sync_done:
